...
def usage():
    ...
    print "-l --listen      - listen on [host]: port for incoming communications"
    ...
    print "echo 'ABCDEFGHI' | ./bphnet.py -t 192.168.0.1 -p 135"
    sys.exit(0)

def server_loop():
    global target
    
    #if no defined target, listen on all interfaces
    if not len(target):
        target = "0.0.0.0"
...
def main():
    ...
    #Check if listening or sending data from stdin
    if not listen and len(target) and port > 0:
        #read buffer from commandline, ctrl d if blocking
